...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="collegeList" title="学院" actionUrl="collegeController.do?datagrid" idField="id" fit="true" queryMode="group">
   ...
   <t:dgDelOpt title="删除" url="collegeController.do?del&id={id}" urlclass="ace_button" urlStyle="background-color:#ec4758;"  urlfont="fa-trash-o"/>
   <t:dgToolBar title="录入" icon="icon-add" url="collegeController.do?addorupdate" funname="add"></t:dgToolBar>
   <t:dgToolBar title="编辑" icon="icon-edit" url="collegeController.do?addorupdate" funname="update"></t:dgToolBar>
   <t:dgToolBar title="查看" icon="icon-search" url="collegeController.do?addorupdate" funname="detail"></t:dgToolBar>
  </t:datagrid>
  </div>
 </div>
 <script>
	...
	function handleApply(id, type, msg){
		layer.open({
			...
			yes:function(index){
				if(id && type){
					$.ajax({
						url: 'collegeController.do?handleApply&id='+id+'&type='+type,
						type: 'POST',
						dataType: 'json',
						success: function(data){
							$('#collegeList').datagrid('reload');
						},
						error: function(data){
