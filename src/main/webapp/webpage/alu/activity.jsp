					...
					<tr>
						...
						<td class="value">
			               <t:dictSelect field="collegeId" id="collegeId"  extendJson="{datatype:'*'}" dictTable="t_college" dictField="id" 
									dictText="name" dictCondition=" where delete_flag=0" defaultVal="${activityPage.collegeId}"  hasLabel="false"></t:dictSelect>
						    <input type="hidden" id="collegeName" name="collegeName"   value="${activityPage.collegeName}" />
				...
				<tr>
					<td align="right">
					    <c:if test="${ not empty LOCAL_CLINET_USER.collegeName  }">
							<input type="hidden" id="collegeId" name="collegeId" value="${activityPage.collegeId}"/>
						</c:if>
